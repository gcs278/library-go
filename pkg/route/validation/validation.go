...
func removeMatchingQuotes(s string) string {
	length := len(s)
	if length < 2 {
		return s // Not enough characters to have matching quotes
	}

	// Check if the first and last characters are single quotes and are not escaped
	if s[0] == '\'' && s[length-1] == '\'' && (length == 2 || s[1] != '\\' && s[length-2] != '\\') {
		return s[1 : length-1] // Remove the quotes
	}
